...
contract("Staking Contract",async(accounts)=>{
  ...
  before(async()=>{
    ...
    let c = await StakingContract.new();
    let d = await StakeTokenNFT.new();
    contractFactory = await ContractFactory.new(a.address,c.address);
    usdc = await USDC.new();
    bloq = await USDC.new();
    ...
    await contractFactory.delpoyStakeContract(d.address,accounts[1],tokenAddress,usdc.address,bloq.address,5,5,"Stake NFT","SN");
    console.log("asdsa");
    stakeAddress = await contractFactory.getStakeAddress(tokenAddress);
    stake = await StakingContract.at(stakeAddress);
    stakeNftAddress = await stake.nftAddress();
    stakeNft = await StakeTokenNFT.at(stakeNftAddress);
    usdc.transfer(stakeAddress,100000);
    bloq.transfer(stakeAddress,100000);
  })
